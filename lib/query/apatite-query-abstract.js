...
class ApatiteQueryAbstract {
    ...
    expandOneToOneExpressions() {
        ...
        this.whereExpressions.forEach(function (eachExpr) {
            if (eachExpr.query && eachExpr.query.isSubQuery)
                eachExpr.expandOneToOneExpressions();
    ...
    execute(onExecuted) {
        if (!this.session)
            throw new ApatiteError('There is no session associated with the query. Use execute on session.');
