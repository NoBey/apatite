...
class ApatiteSession {
    ...
    createDBTablesForAllModels(onCreated) {
        var descriptorSQLs = this.getSQLScriptCreator().createScriptForAllModels();
        var stmts = [];
        descriptorSQLs.forEach(function(eachSQLStr) {
    ...
    createDBTableForModel(modelOrModelName, onCreated) {
        var sqlStr = this.createSQLScriptForModel(modelOrModelName);
        var stmt = new ApatiteDDLSQLStatement(null, sqlStr, []);

        return this.basicExecuteStatementsForCreateTable([stmt], onCreated);
    }

    createDBColumnForAttribute(modelOrModelName, attributeName, onCreated) {
        var sqlStr = this.createSQLScriptForAttribute(modelOrModelName, attributeName);
        var stmt = new ApatiteDDLSQLStatement(null, sqlStr, []);
