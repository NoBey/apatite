...
class ApatiteSession {
    ...
    execute(query, onExecuted) {
        ...
        if (!query.isInternalQuery && query.hasOnlyPrimaryKeyExpressions()) {
    ...
    putNewObjectsInCache(objects) {
        var self = this;
        objects.forEach(function (eachObject) {
            var modelName = eachObject.constructor.name;
            var descriptor = self.apatite.getModelDescriptor(modelName);
            var cacheKey = descriptor.buildCacheKeyFromObject(eachObject);
            self.putObjectInCache(modelName, cacheKey, eachObject);
        });
    }
    ...
    doChangesAndSave(changesToDo, onChangesSaved) {
        this.changesAndSaveQueue.push([changesToDo, onChangesSaved]);
    ...
    processChangesAndSaveQueue() {
        ...
        if (this.trackingChanges)
            throw new ApatiteError('Previous changes have not been saved. Probably the callback done() of changesToDo parameter of method doChangesAndSave(changesToDo, onChangesSaved) is not called.');

        var queueInfo = this.changesAndSaveQueue.shift();
        this.startTrackingChanges();
        var self = this;
        var changesToDo = queueInfo[0];
        var onChangesSaved =  queueInfo[1];

        changesToDo(function (err) {
            if (err) {
                self.rollbackChanges();
                onChangesSaved(err);
            }
            else
                self.saveChanges(onChangesSaved);
        });
    }
